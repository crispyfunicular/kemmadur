...
def play(bdd: str) -> None:
    ...
    console.print(
        "[bold blue]Degemer mat ! Bienvenue dans le test de mutations bretonnes.[/bold blue]"
    )

    # Chargement des requêtes depuis requetes.sql
    queries = aiosql.from_path("requetes.sql", "sqlite3")

    # Exécution de la requête
    try:
        flashcards = list(
            queries.article_nom_adj(conn)
        )  # Renvoie une liste : [('mamm + brav = ?', 'mamm vrav'), ...]
    except sqlite3.Error as e:
        console.print(f"[red]Erreur SQL : {e}[/red]")
        conn.close()
        return
    ...
    for question, reponse_attendue in flashcards:
        ...
        if reponse_user.strip().lower() == reponse_attendue.lower():
            ...
            console.print(
                f"[bold red]Fazi...[/bold red] La bonne réponse était : [bold green]{reponse_attendue}[/bold green]\n"
            )

    # Fin de la partie
    console.print("---")
    console.print(
        f"[bold cyan]Fin de l'exercice ! Votre score : {score} / {total}[/bold cyan]"
    )
